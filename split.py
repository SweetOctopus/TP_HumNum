...
#Comment VP
#Le split, c'est decouper le corpus en (dans le cas présent) 80% de train, 10 de dev et 10 de test
# on va également sortir ce qui est considéré "out of domain" peut-être des absrudité graphique, 
#des fautes ou des choses trop rares pour avoir un sens d'être inclu dans le modèle
...
#Comment VP
#c'est ici que je pourrais add mes données, si je voulais ajouter un corpus de francais moderne pour apprendre à ne PAS normaliser
#du style-> "7-modern":["train","train","train","train","train","train","train","train","train","train"], (je crois qu'on doit tout mettre dans le train)
#avec l'idée train -> pour entrainer le modèle, dev->pour que le modèle ait une base, test-> pour tester le modele, il ne va pas modifier les poids selon test
# out -> out of domain
